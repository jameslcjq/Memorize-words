...
// Utility to determine which indices to mask - ALL letters for dictation
...
interface SmartDictationGameProps {
  /** 当前单词 */
  word: Word
  /** 完成回调（答对） */
  onComplete: () => void
...
const SmartDictationGame: React.FC<SmartDictationGameProps> = ({ word, onComplete, onError }) => {
  ...
  useAnswerHintAdvance({
    ...
    onSuccess: () => onCompleteRef.current(),
    onWrong: () => onErrorRef.current(),
  })
  ...
  useEffect(() => {
    ...
    const handlePointerUp = (e: PointerEvent) => {
      ...
      const target = elements.find((el) => el.hasAttribute('data-dictation-index'))

      if (target) {
        const indexStr = target.getAttribute('data-dictation-index')
        if (indexStr) {
  ...
  return (
    <div className="flex w-full flex-col items-center justify-center gap-4 py-4 md:gap-6 md:py-6 lg:gap-8 lg:py-10">
      ...
      <div className="flex flex-col items-center gap-4">
        <button
          onClick={() => playWordRef.current()}
          className="flex h-24 w-24 items-center justify-center rounded-full bg-indigo-500 text-white shadow-lg transition-all hover:scale-105 hover:bg-indigo-600 active:scale-95"
          title="播放发音"
        >
          <IconVolume className="h-12 w-12" />
        </button>
        <p className="text-sm text-gray-500 dark:text-gray-400">点击播放发音</p>
      </div>
      ...
      <div className="relative w-full">
        ...
        >
          {currentWordObj.name.split('').map((char, index) => {
            ...
            return (
              <div key={index} className="relative" data-dictation-index={isMasked ? index : undefined}>
                <input
...
export default SmartDictationGame
