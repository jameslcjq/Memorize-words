...
// .wrangler/tmp/bundle-9wPHPl/checked-fetch.js
var urls = /* @__PURE__ */ new Set()
function checkURL(request, init) {
  const url = request instanceof URL ? request : new URL((typeof request === 'string' ? new Request(request, init) : request).url)
...
// .wrangler/tmp/pages-rCJtoG/functionsWorker-0.10004323139854021.mjs
...
var onRequestGet = /* @__PURE__ */ __name2(async (context) => {
  ...
  try {
    ...
    const activeCount = await env.DB.prepare('SELECT COUNT(DISTINCT user_id) as count FROM study_records WHERE updated_at > ?')
      .bind(oneDayAgo)
      .first('count')
    return new Response(
      JSON.stringify({
...
var onRequestPost = /* @__PURE__ */ __name2(async (context) => {
  ...
  try {
    ...
    if (type === 'register') {
      ...
      await env.DB.prepare('INSERT INTO users (id, username, password, nickname, created_at) VALUES (?, ?, ?, ?, ?)')
        .bind(userId, username, hashedPassword, nickname || 'User', Date.now())
        .run()
      return new Response(
        JSON.stringify({
          success: true,
...
// .wrangler/tmp/bundle-9wPHPl/middleware-insertion-facade.js
var __INTERNAL_WRANGLER_MIDDLEWARE__2 = [middleware_ensure_req_body_drained_default2, middleware_miniflare3_json_error_default2]
...
// .wrangler/tmp/bundle-9wPHPl/middleware-loader.entry.ts
...
export { __INTERNAL_WRANGLER_MIDDLEWARE__2 as __INTERNAL_WRANGLER_MIDDLEWARE__, middleware_loader_entry_default2 as default }
//# sourceMappingURL=functionsWorker-0.10004323139854021.js.map
